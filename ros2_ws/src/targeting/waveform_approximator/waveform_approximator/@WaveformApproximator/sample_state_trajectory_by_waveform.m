function sampling_points = sample_state_trajectory_by_waveform(obj, state_trajectory, target_waveform, num_of_intermediate_points_per_mode)
    ...
    for i = 1:length(target_waveform)
        ...
        if i == 1
            position = 1;
        elseif i == length(target_waveform)
            position = 3;
        else
            position = 2;
        end
        ...
        for j = 1:length(sampling_times)
            ...
            mode_info = struct('current_mode', current_mode, 'mode_index', i, 'segment_index', j, 'is_last_segment', is_last_segment, 'position', position);
