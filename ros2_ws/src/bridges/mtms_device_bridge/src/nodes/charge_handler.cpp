...
ChargeHandler::ChargeHandler() : Node("charge_handler") {
  ...
  send_charge_subscriber_ = this->create_subscription<event_interfaces::msg::Charge>(
      "/event/send/charge", 10, std::bind(&ChargeHandler::charge_callback, this, std::placeholders::_1));
}

// Implementation of the callback method
void ChargeHandler::charge_callback(const std::shared_ptr<event_interfaces::msg::Charge> charge) {
  ...
  /* Log charge message. */
  RCLCPP_INFO(rclcpp::get_logger("charge_handler"), "Charging channel %d to %d V (id: %d, execution_condition: %d, execution_time: %.4f s)",
              charge->channel,
