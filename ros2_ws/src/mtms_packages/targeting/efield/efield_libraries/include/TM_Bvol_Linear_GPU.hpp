...
/**
 * @brief weighs DB by conductivities and multiplies the result by TM. Uses GPU for the multiplication
 * 
 ...
 */
Eigen::MatrixXf TM_Bvol_Linear_GPU(const std::vector<Eigen::MatrixXf> &DB, const Eigen::MatrixXf &TM,  const std::vector<float> &ci, const std::vector<float> &co);
