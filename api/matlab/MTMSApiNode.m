classdef MTMSApiNode < handle
    ...
    methods
        function obj = MTMSApiNode(channel_count)
            ...
            % Wait for all service servers to become available before returning.
            % Each client may belong to a different ROS node, so all must be waited on independently.
            clients = { ...
                obj.start_device_client, ...
                obj.stop_device_client, ...
                obj.request_events_client, ...
                obj.trigger_events_client, ...
                obj.start_session_client, ...
                obj.stop_session_client, ...
                obj.get_target_voltages_client, ...
                obj.get_maximum_intensity_client, ...
                obj.get_default_waveform_client, ...
                obj.get_multipulse_waveforms_client, ...
                obj.reverse_polarity_client, ...
                obj.analyze_mep_client, ...
                obj.get_trigger_window_client, ...
            };
            for i = 1:numel(clients)
                fprintf('Waiting for service: %s\n', clients{i}.ServiceName);
                waitForServer(clients{i});
            end
            fprintf('All services ready.\n');
        end
