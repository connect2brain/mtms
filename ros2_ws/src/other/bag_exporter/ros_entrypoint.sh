...
if [ -z "$BAG_ANALYSIS_TIMESTAMP" ]; then
  python3 bag_exporter.py --bag $BAG_ANALYSIS_NAME --topic $BAG_ANALYSIS_TOPIC --full
else
  python3 bag_exporter.py --bag $BAG_ANALYSIS_NAME --timestamp $BAG_ANALYSIS_TIMESTAMP --topic $BAG_ANALYSIS_TOPIC --full
fi
