...
DischargeHandler::DischargeHandler() : Node("discharge_handler") {
  ...
  send_discharge_subscriber_ = this->create_subscription<event_interfaces::msg::Discharge>(
      "/event/send/discharge", 10, std::bind(&DischargeHandler::discharge_callback, this, std::placeholders::_1));
}

void DischargeHandler::discharge_callback(const std::shared_ptr<event_interfaces::msg::Discharge> discharge) {
  ...
  RCLCPP_INFO(rclcpp::get_logger("discharge_handler"), "Discharging channel %d to %d V (id: %d, execution_condition: %d, execution_time: %.4f s)",
              discharge->channel,
  ...
  if (execution_time < 0.0) {
    RCLCPP_ERROR(rclcpp::get_logger("discharge_handler"), "Execution time cannot be negative, aborting discharge (id: %d)", id);
    return;
  }
