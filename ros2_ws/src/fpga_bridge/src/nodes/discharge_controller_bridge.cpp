...
public:
  DischargeControllerBridge()
    ...
    discharge_controllers_publisher_ = this->create_publisher<fpga_interfaces::msg::DischargeControllerStates>(
        "/fpga/discharge_controller_states", 10);
    timer_ = this->create_wall_timer(20ms,
                                     std::bind(&DischargeControllerBridge::publish_discharge_controller_state, this));
  }
...
int main(int argc, char **argv) {
  ...
  RCLCPP_INFO(rclcpp::get_logger("dischare_controller_bridge"), "Setting thread scheduling and memory lock");
...
#if defined(ON_UNIX) && defined(MEMORY_OPTIMIZATION)
  RCLCPP_INFO(rclcpp::get_logger("dischare_controller_bridge"), "Locking memory");
  lock_memory();
