...
bool init_fpga(void) {
  ...
  if (NiFpga_IsError(status)) {
    RCLCPP_INFO(rclcpp::get_logger("discharge_controller_bridge"), "FPGA could not be initialized, exiting.");
    return false;
  }
  ...
  if (NiFpga_IsError(status)) {
      RCLCPP_INFO(rclcpp::get_logger("discharge_controller_bridge"), "FPGA bitfile could not be loaded, exiting.");
      return false;
  }
